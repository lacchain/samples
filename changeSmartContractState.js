...
const setRecipientContract = () => { //Set for RelayHub Contract
  console.log("#######################Preparing value for Recipient SmartContract #######################")
  const functionName = "transfer" //store(uint256)
  const recipientData = "address"	
  const typeOfData = "uint256"

  // store(uint256)

  const recipient = "0x173CF75f0905338597fcd38F5cE13E6840b230e9"	
  const number = 1
  const nodeAddress = "0xd00e6624a73f88b39f82ab34e8bf2b4d226fd768"
  const expiration = 1636394529

  let set = web3.eth.abi.encodeFunctionSignature(`${functionName}(${recipientData},${typeOfData})`)//function name to use

  let value = web3.eth.abi.encodeParameters(
    ["address","uint256","address","uint256"],
    [recipient,number,nodeAddress,expiration])//setting the value
  const txData = set + value.substr(2)
  console.log("txData:"+txData)
  return txData
}
